...
@pytest.fixture
def valid_scriv_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Ui' if (network == 'testnet') else 'Ad6FonUgjNgck2dHpCeaBgaFpPjSMX2say'


@pytest.fixture
def invalid_scriv_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Uj' if (network == 'testnet') else 'Ad6FonUgjNgck2dHpCeaBgaFpPjSMX2sam'
...
def test_valid_scriv_address():
    from scrivlib import is_valid_scriv_address

    main = valid_scriv_address()
    test = valid_scriv_address('testnet')

    assert is_valid_scriv_address(main) is True
    assert is_valid_scriv_address(main, 'mainnet') is True
    assert is_valid_scriv_address(main, 'testnet') is False

    assert is_valid_scriv_address(test) is False
    assert is_valid_scriv_address(test, 'mainnet') is False
    assert is_valid_scriv_address(test, 'testnet') is True


def test_invalid_scriv_address():
    from scrivlib import is_valid_scriv_address

    main = invalid_scriv_address()
    test = invalid_scriv_address('testnet')

    assert is_valid_scriv_address(main) is False
    assert is_valid_scriv_address(main, 'mainnet') is False
    assert is_valid_scriv_address(main, 'testnet') is False

    assert is_valid_scriv_address(test) is False
    assert is_valid_scriv_address(test, 'mainnet') is False
    assert is_valid_scriv_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from scrivlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % scrivlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(scrivlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(scrivlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(scrivlib.blocks_to_seconds(16616)) == 2612035
