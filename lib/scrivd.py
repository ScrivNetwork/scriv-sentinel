...
class ScrivDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some scrivd methods
    ...
    @classmethod
    def from_scriv_conf(self, scriv_dot_conf):
        from scriv_config import ScrivConfig
        config_text = ScrivConfig.slurp_config_file(scriv_dot_conf)
        creds = ScrivConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = scrivlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = scrivlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = scrivlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_scrivd_proto_version_with_sentinel_ping)
