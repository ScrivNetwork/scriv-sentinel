...
class ScrivConfig():
    ...
    def slurp_config_file(self, filename):
        # read scriv.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is scriv.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        # convert to an int if taken from scriv.conf
