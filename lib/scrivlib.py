...
def is_valid_scriv_address(address, network='mainnet'):
    ...
    scriv_version = 140 if network == 'testnet' else 23
    ...
    try:
        ...
        # rescue from exception, not a valid scriv address
        return False

    if (address_version != scriv_version):
        return False
...
# shims 'til we can fix the scrivd side
def SHIM_serialise_for_scrivd(sentinel_hex):
    from models import scrivd_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = scrivd_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in scrivd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # scrivd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    scrivd_hex = serialise(obj)
    return scrivd_hex


# shims 'til we can fix the scrivd side
def SHIM_deserialise_from_scrivd(scrivd_hex):
    from models import scrivd_GOVOBJ_TYPES

    # unpack
    obj = deserialise(scrivd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('argo.conf')
