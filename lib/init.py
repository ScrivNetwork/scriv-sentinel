...
def has_scriv_conf():
    ...
    valid_scriv_conf = False

    # ensure scriv_conf exists & readable
    #
    # if not, print a message stating that Scriv Core must be installed and
    # configured, including JSONRPC access in scriv.conf
    try:
        f = io.open(config.scriv_conf)
        valid_scriv_conf = True
    except IOError as e:
        print(e)

    return valid_scriv_conf
...
def main():
    ...
    if not has_scriv_conf():
        print("ScrivCore must be installed and configured, including JSONRPC access in scriv.conf")
        sys.exit(1)
