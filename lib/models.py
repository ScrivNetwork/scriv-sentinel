...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, scrivd):
        golist = scrivd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_scrivd(scrivd, item)
    ...
    @classmethod
    def import_gobject_from_scrivd(self, scrivd, rec):
        import decimal
        import scrivlib
        import inflection
        ...
        # shim/scrivd conversion
        object_hex = scrivlib.SHIM_deserialise_from_scrivd(object_hex)
        objects = scrivlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from scrivd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from scrivd, with every run
        ...
        # get/create, then sync payment amounts, etc. from scrivd - scrivd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(scrivd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from scrivd! %s" % e)
            govobj.vote_delete(scrivd)
            return (govobj, None)
    ...
    def vote_delete(self, scrivd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(scrivd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, scrivd, signal, outcome):
        import scrivlib
        ...
        output = scrivd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = scrivlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(scrivd, signal)

    def sync_network_vote(self, scrivd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = scrivd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = scrivd_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 scriv addr, non-multisig
            if not scrivlib.is_valid_scriv_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Scriv address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (scrivlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/scrivdrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import scrivlib
        obj_data = scrivlib.SHIM_serialise_for_scrivd(self.serialise())
    ...
    def prepare(self, scrivd):
        try:
            object_hash = scrivd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = scrivd_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not scrivlib.is_valid_scriv_address(addr, config.network):
    ...
    def hash(self):
        import scrivlib
        return scrivlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = scrivd_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, scrivd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - scrivd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, scrivd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - scrivd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, scrivd):
        now = int(time.time())
        return (self.created_at < (now - scrivd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, scrivd):
        if self.is_expired(scrivd):
            return False

        return True

    def is_deletable(self, scrivd):
        if self.is_expired(scrivd):
            return True
