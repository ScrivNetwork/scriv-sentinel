...
sentinel_cfg = ScrivConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_scrivd_proto_version_with_sentinel_ping = 70207


def get_scriv_conf():
    home = os.environ.get('HOME')

    scriv_conf = os.path.join(home, ".scrivcore/scriv.conf")
    if sys.platform == 'darwin':
        scriv_conf = os.path.join(home, "Library/Application Support/ScrivCore/scriv.conf")

    scriv_conf = sentinel_cfg.get('scriv_conf', scriv_conf)
